...
export function SettingsModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  // global state
  const { apiKey, setApiKey, modelTemperature, setModelTemperature, modelMaxTokens, setModelMaxTokens, modelApiHost, setModelApiHost } = useSettingsStore(
    (state) => ({
      apiKey: state.apiKey,
      setApiKey: state.setApiKey,
      modelTemperature: state.modelTemperature,
      setModelTemperature: state.setModelTemperature,
      modelMaxTokens: state.modelMaxTokens,
      setModelMaxTokens: state.setModelMaxTokens,
      modelApiHost: state.modelApiHost,
      setModelApiHost: state.setModelApiHost,
    }),
    shallow,
  );
      ...
      <ModalDialog sx={{ minWidth: '30vw' }}>
        ...
        <Box sx={{ mt: 3, minWidth: 320 }}>
          <Typography level="body1" sx={{ mb: 1 }}>
            ...
            {needsApiKey ? '(required' : '(optional'}, check{' '}
          ...
          <Typography level="body2" sx={{ mt: 1, mb: 1, lineHeight: 1.75 }}>
            {needsApiKey ? (
              <>
                <Typography variant="soft">sk-...</Typography> Create key, then apply to the{' '}
                <Link href="https://openai.com/waitlist/gpt-4-api" target="_blank">
                  GPT-4 waitlist
                </Link>
              </>
            ) : (
          ...
          {showAdvanced && (
            <>
              <Grid container spacing={1} sx={{ alignItems: 'center', mb: 1 }}>
                ...
                <Grid xs={6} md={7} xl={7}>
                  <Input variant="plain" placeholder="api.openai.com" value={modelApiHost} onChange={handleModelApiHostChange} />
                </Grid>
              </Grid>
