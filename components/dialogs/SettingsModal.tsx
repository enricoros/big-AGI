  ...
  return (
    <>
      <Stack direction="row" sx={{ mt: props.title ? 2 : 2, alignItems: 'center' }}>
        {!!props.title && <Typography level="body2">{props.title}</Typography>}
        {!!props.collapsible && (
      ...
      {!collapsed && <Box sx={{ mt: 1, mb: 1 }}>{props.children}</Box>}
...
export function SettingsModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const {
    ...
    (state) => ({
      ...
      modelMaxResponseTokens: state.modelMaxResponseTokens,
      setModelMaxResponseTokens: state.setModelMaxResponseTokens,
        ...
        <Section title="Advanced AI settings" collapsible collapsed={true} disclaimer="Adjust only if you are familiar with these terms">
          <Stack direction="column" sx={{ gap: 1, mt: -0.8, maxWidth: 400 }}>
            ...
            <FormControl orientation="horizontal" sx={{ justifyContent: 'space-between' }}>
              ...
              <Input variant="outlined" placeholder="api.openai.com" value={modelApiHost} onChange={handleModelApiHostChange} sx={{ flexGrow: 1 }} />
            </FormControl>
          </Stack>
        </Section>
