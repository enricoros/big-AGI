...
export function Composer(props: {
  ...
  const handleOverlayDrop = async (e: React.DragEvent) => {
    ...
    if (droppedText?.length >= 1) return setComposeText((text) => expandPromptTemplate(PromptTemplates.PasteText, { clipboard: droppedText })(text));
        ...
        <Stack direction="row" spacing={{ xs: 1, md: 2 }}>
          ...
          <Box sx={{ flexGrow: 1, position: 'relative' }}>
            ...
            <Badge
              ...
              badgeContent={
                estimatedTokens > 0 ? (
                  <Tooltip title={tokensString} color={tokenColor}>
            ...
            {isSpeechEnabled && (
              <IconButton
                variant={!isRecordingSpeech ? 'plain' : 'solid'}
                color={!isRecordingSpeech ? 'primary' : 'warning'}
      ...
      <Grid xs={12} md={3}>
        <Stack spacing={2}>
          <Box sx={{ display: 'flex', flexDirection: 'row' }}>
            ...
            <Button
              fullWidth
              variant={props.disableSend ? 'soft' : 'solid'}
              color="primary"
