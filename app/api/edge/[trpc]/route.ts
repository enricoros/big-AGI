...
const handlerMainRoutes = (req: Request) => fetchRequestHandler({
  endpoint: '/api/edge',
  ...
  onError:
    process.env.NODE_ENV === 'development'
      ? ({ path, error }) => console.error(`\n❌ tRPC main failed on ${path ?? 'unk-path'}: ${error.message}`)
      : undefined,
});
...
export { handlerMainRoutes as GET, handlerMainRoutes as POST };
