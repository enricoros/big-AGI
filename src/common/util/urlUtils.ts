...
import { isBrowser, isVercelFromBackend } from './pwaUtils';
...
export function getBaseUrl(): string {
  ...
  if (isVercelFromBackend) return `https://${process.env.VERCEL_URL}`; // SSR should use vercel url
...
export function getOriginUrl(): string {
  ...
  if (isVercelFromBackend) return `https://${process.env.VERCEL_URL}`;
