...
export type DMessageUserFlag = 'starred'; // user starred this
...
export function messageToggleUserFlag(
  message: DMessage,
  flag: DMessageUserFlag
): DMessageUserFlag[] {
  if (message.userFlags?.includes(flag)) return message.userFlags.filter((_f) => _f !== flag);
  else return [...(message.userFlags || []), flag];
}
...
export interface ChatActions {
  ...
  ) => DConversationId | null;
  deleteConversations: (
    conversationIds: DConversationId[],
    newConversationPersonaId?: SystemPurposeId
  ) => DConversationId;
  ...
  updateMetadata: (
    conversationId: string,
    messageId: string,
    metadataDelta: Partial<DMessageMetadata>,
    touchUpdated?: boolean
  ) => void;
  ...
  _editConversation: (
    conversationId: string,
    update: Partial<DConversation> | ((conversation: DConversation) => Partial<DConversation>)
  ) => void;
}
    ...
    persist(
      (_set, _get) => ({
        ...
        importConversation: (
          ...
          conversation.tokenCount = updateTokenCounts(
        ...
        branchConversation: (
          ...
          const deepCopy = JSON.parse(JSON.stringify(conversation)) as DConversation;
          ...
          const branched: DConversation = {
            ...
            messages: deepCopy.messages.slice(0, messageIndex).map(
        ...
        deleteConversations: (
          ...
          const cIndex =
            conversationIds.length > 0
              ? conversations.findIndex((_c) => _c.id === conversationIds[0])
              : -1;

          // abort all pending requests
          conversationIds.forEach((conversationId) =>
            conversations.find((_c) => _c.id === conversationId)?.abortController?.abort()
          );
...
function updateTokenCounts(messages: DMessage[], forceUpdate: boolean, debugFrom: string): number {
  ...
  return (
    ...
    messages.reduce(
      (sum, message) =>
        4 + updateDMessageTokenCount(message, chatLLMId, forceUpdate, debugFrom) + sum,
      0
    )
  );
}
...
export const getConversationSystemPurposeId = (
  conversationId: DConversationId | null
): SystemPurposeId | null => getConversation(conversationId)?.systemPurposeId || null;
...
export const useConversation = (conversationId: DConversationId | null) =>
  useChatStore((state) => {
    ...
    const conversation = conversationId
      ? conversations.find((_c) => _c.id === conversationId) ?? null
      : null;
    ...
    const conversationIdx = conversation
      ? conversations.findIndex((_c) => _c.id === conversation.id)
      : -1;

    const hasConversations =
      conversations.length > 1 ||
      (conversations.length === 1 && !!conversations[0].messages.length);
    const recycleNewConversationId =
      conversations.length && !conversations[0].messages.length ? conversations[0].id : null;
