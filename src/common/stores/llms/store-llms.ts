...
export const useModelsStore = create<LlmsStore>()(persist(
  ...
  }),
  {
     ...
     *  2: large changes on all LLMs, and reset chat/fast/func LLMs (pre-FylloAI)
     *  3: FylloAI v2 (formerly big-AGI v2)
    ...
    migrate: (_state: any, fromVersion: number): LlmsStore => {
      ...
      // 2 -> 3: FylloAI v2 (formerly big-AGI v2): update all models for pricing info
      if (fromVersion < 3) {
        try {
