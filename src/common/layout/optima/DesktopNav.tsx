...
export function DesktopNav(props: { component: React.ElementType; currentApp?: NavItemApp }) {
  ...
  const { showPreferencesTab, openPreferencesTab, showModelsSetup, openModelsSetup } =
    useOptimaLayout();
  ...
  const handleLogoButtonClick = React.useCallback(() => {
    if (logoButtonTogglesPane) toggleDrawer();
      ...
      components.push(
        <Dropdown key="n-app-overflow">
          <Tooltip disableInteractive enterDelay={600} title="More Apps">
            <MenuButton
              slots={{ root: DesktopNavIcon }}
              slotProps={{ root: { className: navItemClasses.typeApp } }}
            >
              <MoreHorizIcon />
            </MenuButton>
          </Tooltip>
  ...
  const navModalItems = React.useMemo(() => {
    return navItems.modals.map((item) => {
      ...
      return (
        <Tooltip
          followCursor
          key={'n-m-' + item.overlayId}
          title={isAttractive ? 'Add Language Models - REQUIRED' : item.name}
        >
          <DesktopNavIcon
        ...
        <Tooltip title={isDrawerOpen ? 'Close Drawer' /* for Aria reasons */ : 'Open Drawer'}>
          ...
          >
            {logoButtonTogglesPane ? (
              ...
              <AgiSquircleIcon inverted sx={{ color: 'white' }} />
            )}
          </DesktopNavIcon>
        </Tooltip>
      </InvertedBarCornerItem>
