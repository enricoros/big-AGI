...
export async function runImageGenerationUpdatingState(cHandler: ConversationHandler, imageText: string, imageFragments?: Immutable<DMessageFragment[]>) {
  ...
  try {
    const imageContentFragments = await t2iGenerateImageContentFragments(t2iProvider, imageText, aixInlineImageParts, repeat, 'app-chat', { t2iContextName: 'chat-draw-command' });
