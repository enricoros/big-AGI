...
const DEBUG_PANES_MANAGER = true;
...
const useAppChatPanesStore = create<AppChatPanesStore>()(
  persist(
    (_set, _get) => ({
      ...
      openConversationInFocusedPane: (conversationId: DConversationId) => {
        _set((state) => {
          ...
          if (!focusedPane) {
            console.warn(
              'openConversationInFocusedPane: focusedPane is null',
              chatPaneFocusIndex,
              chatPanes
            );
            return state;
          ...
          if (focusedPane.conversationId === conversationId) {
            if (DEBUG_PANES_MANAGER)
              console.log(
                `open-focuses: ${conversationId} is open in focused pane`,
                chatPaneFocusIndex,
                chatPanes
              );
            return state;
          ...
          if (DEBUG_PANES_MANAGER)
            console.log(
              `open-focuses: set ${conversationId} in focused pane`,
              chatPaneFocusIndex,
              chatPanes
            );
      ...
      openConversationInSplitPane: (conversationId: DConversationId) => {
        ...
        const focusedPane =
          chatPaneFocusIndex !== null ? chatPanes[chatPaneFocusIndex] ?? null : null;

        // if fewer than the maximum panes, create a new pane and focus it
        if (chatPanes.length < MAX_CONCURRENT_PANES) {
          const insertIndex =
            chatPaneFocusIndex !== null ? chatPaneFocusIndex + 1 : chatPanes.length;
          _set((state) => ({
            chatPanes: [
        ...
        // max reached, replace the next pane (with wraparound) - note the outside logic won't get us here
          ...
          const replaceIndex =
            (chatPaneFocusIndex !== null ? chatPaneFocusIndex + 1 : 0) % MAX_CONCURRENT_PANES;
          _set({
        ...
        if (DEBUG_PANES_MANAGER) console.log(`open-split-pane: after:`, _get().chatPanes);
      },

      navigateHistoryInFocusedPane: (direction: 'back' | 'forward'): boolean => {
        const { chatPanes, chatPaneFocusIndex } = _get();
        if (chatPaneFocusIndex === null) return false;

        const focusedPane = chatPanes[chatPaneFocusIndex];
        let newHistoryIndex = focusedPane.historyIndex;

        if (direction === 'back' && newHistoryIndex > 0) newHistoryIndex--;
      ...
      removeOtherPanes: () =>
        _set((state) => {
          const { chatPanes, chatPaneFocusIndex } = state;
          if (chatPanes.length < 2) return state;
      ...
      removePane: (paneIndex: number) =>
        _set((state) => {
          ...
          if (paneIndex < 0 || paneIndex >= chatPanes.length) return state;
      ...
      setFocusedPaneIndex: (paneIndex: number) =>
        _set((state) => {
          if (state.chatPaneFocusIndex === paneIndex) return state;
          return {
            chatPaneFocusIndex:
              paneIndex >= 0 && paneIndex < state.chatPanes.length ? paneIndex : null,
          };
        }),
      ...
      _onConversationsChanged: (conversationIds: DConversationId[]) =>
        _set((state) => {
          ...
          // handle panes
          let untouched = true;
          const newPanes: ChatPane[] = chatPanes
            .map((chatPane) => {
              ...
              const newHistory = history.filter((_hId, index) => {
                ...
                if (!historyStillPresent && index <= historyIndex) newHistoryIndex--;
                return historyStillPresent;
              });
              if (newHistoryIndex < 0 && newHistory.length > 0) newHistoryIndex = 0;

              // check if pointing to a valid conversationId
              const needsNewConversationId =
                !conversationId || !conversationIds.includes(conversationId);
              if (!needsNewConversationId && newHistory.length === history.length) return chatPane;

              const nextConversationId =
                newHistoryIndex >= 0 && newHistoryIndex < newHistory.length
          ...
          if (untouched && newPanes.length >= 1) return state;
          ...
          return {
            ...
            chatPaneFocusIndex:
              newPanes.length && chatPaneFocusIndex !== null && chatPaneFocusIndex < newPanes.length
                ? chatPaneFocusIndex
                : 0,
          };
        }),
...
export function usePanesManager() {
  // use Panes
  const { _onConversationsChanged, ...panesFunctions } = useAppChatPanesStore(
    useShallow((state) => ({
      // state
      chatPanes: state.chatPanes as Readonly<ChatPane[]>,
      focusedPaneIndex: state.chatPaneFocusIndex,
      focusedPaneConversationId:
        state.chatPaneFocusIndex !== null
  ...
  const conversationIDs: DConversationId[] = useChatStore(
    useShallow((state) => state.conversations.map((_c) => _c.id))
  );
