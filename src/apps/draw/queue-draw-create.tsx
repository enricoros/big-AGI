...
const drawCreateWorker: ItemAsyncWorker<DesignerPrompt> = async (item, _update, signal) => {
  await t2iGenerateImageContentFragments(null, item.prompt, [], item._repeatCount, 'app-draw', { t2iContextName: 'draw-app-queue', abortSignal: signal }).catch(console.error);
  return item;
};
