...
import { BRay, rayHasMergeableContent, rayIsScattering } from '../scatter/beam.scatter';
...
export function gatherInputsFromRays(rays: BRay[]): { messages: DMessage[], pendingCount: number } {
  return {
    // the replies to merge: settled, with content (not empty, not just an error)
    messages: rays.filter(ray => !rayIsScattering(ray) && rayHasMergeableContent(ray)).map(ray => ray.message),
