...
import {
  ...
  wireOpenAICreateImageOutputSchema,
  WireOpenAICreateImageRequest,
} from './openai.wiretypes';
import {
...
import {
  llmsChatGenerateWithFunctionsOutputSchema,
  llmsGenerateContextSchema,
  llmsListModelsOutputSchema,
  ModelDescriptionSchema,
} from '../llm.server.types';
import {
  wilreLocalAIModelsApplyOutputSchema,
...
const createImagesInputSchema = z.object({
  ...
  config: z.object({
    ...
    responseFormat: z.enum([
      'url',
      'b64_json',
    ]) /* udpated to directly match OpenAI's formats - shall have an intermediate representation instead? */,
...
export const llmOpenAIRouter = createTRPCRouter({
  ...
  listModels: publicProcedure
    ...
    .query(async ({ input: { access } }): Promise<{ models: ModelDescriptionSchema[] }> => {
      ...
      if (access.dialect === 'azure') {
        const azureModels = await openaiGETOrThrow(
          access,
      ...
      const openAIWireModelsResponse = await openaiGETOrThrow<OpenAIWire.Models.Response>(
        access,
        '/v1/models'
      );
      ...
      openAIModels = openAIModels.filter(
        (model, index) => openAIModels.findIndex((m) => m.id === model.id) === index
      );
      if (preCount !== openAIModels.length)
        console.warn(
          `openai.router.listModels: removed ${preCount - openAIModels.length} duplicate models for dialect ${access.dialect}`
        );
      ...
      switch (access.dialect) {
        ...
        case 'lmstudio':
          models = openAIModels.map(({ id }) => lmStudioModelToModelDescription(id));
          break;
        ...
        case 'mistral':
          models = openAIModels.map(mistralModelToModelDescription).sort(mistralModelsSort);
          break;
        ...
        case 'openai':
          models = openAIModels
            ...
            .map(
              (model): ModelDescriptionSchema =>
                openAIModelToModelDescription(model.id, model.created)
            )
  ...
  chatGenerateWithFunctions: publicProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      const completionsBody = openAIChatCompletionPayload(
        access.dialect,
        model,
        history,
        isFunctionsCall ? functions : null,
        forceFunctionName ?? null,
        1,
        false
      );
      const wireCompletions = await openaiPOSTOrThrow<
        OpenAIWire.ChatCompletion.Response,
      ...
      if (wireCompletions?.choices?.length !== 1) {
        console.error(
          `[POST] llmOpenAI.chatGenerateWithFunctions: ${access.dialect}: ${context?.name || 'no context'}: unexpected output${forceFunctionName ? ` (fn: ${forceFunctionName})` : ''}:`,
          model.id,
          wireCompletions?.choices
        );
        throw new TRPCError({
      ...
      return finish_reason === 'function_call' || 'function_call' in message
        ? parseChatGenerateFCOutput(
            isFunctionsCall,
            message as OpenAIWire.ChatCompletion.ResponseFunctionCall
          )
        : parseChatGenerateOutput(
            message as OpenAIWire.ChatCompletion.ResponseMessage,
            finish_reason
          );
    }),
  ...
  createImages: publicProcedure
    ...
    .mutation(async ({ input: { access, config } }) => {
      ...
      if (config.model === 'dall-e-3' && config.count > 1)
        throw new TRPCError({
          code: 'BAD_REQUEST',
          message: `[OpenAI Issue] dall-e-3 model does not support more than 1 image`,
        });
      ...
      if (access.dialect === 'localai') delete requestBody.response_format;

      // create 1 image (dall-e-3 won't support more than 1, so better transfer the burden to the client)
      const wireOpenAICreateImageOutput = await openaiPOSTOrThrow<
        WireOpenAICreateImageOutput,
        WireOpenAICreateImageRequest
      ...
      return imagesOutput.data.map((image) => {
        if ('b64_json' in image)
          throw new TRPCError({
            ...
            message: `[OpenAI Issue] Expected a url, got a b64_json (which is not implemented yet)`,
          });
        return {
          imageUrl: image.url,
  ...
  moderation: publicProcedure
    ...
    .mutation(async ({ input: { access, text } }): Promise<OpenAIWire.Moderation.Response> => {
      try {
        return await openaiPOSTOrThrow<
          OpenAIWire.Moderation.Response,
          OpenAIWire.Moderation.Request
        >(
          access,
          null,
          {
        ...
        if (error.code === 'ECONNRESET')
          throw new TRPCError({
            ...
            message: 'Connection reset by the client.',
          });
  ...
  dialectLocalAI_galleryModelsApply: publicProcedure
    ...
    .mutation(async ({ input: { access, galleryName, modelName } }) => {
      ...
      const wireLocalAIModelApply = await openaiPOSTOrThrow(
        access,
        null,
        { id: galleryModelId },
        '/models/apply'
  ...
  switch (access.dialect) {
    case 'azure':
      ...
      if (!azureKey || !azureHost)
        throw new Error(
          'Missing Azure API Key or Host. Add it on the UI (Models Setup) or server side (your deployment).'
        );
    ...
    case 'openai':
      ...
      let oaiHost = fixupHost(
        access.oaiHost || env.OPENAI_API_HOST || DEFAULT_OPENAI_HOST,
        apiPath
      );
      // warn if no key - only for default (non-overridden) hosts
      if (!oaiKey && oaiHost.indexOf(DEFAULT_OPENAI_HOST) !== -1)
        throw new Error(
          'Missing OpenAI API Key. Add it on the UI (Models Setup) or server side (your deployment).'
        );
      ...
      if (oaiHost.includes('https://gateway.ai.cloudflare.com')) {
        ...
        if (pathSegments.length < 3 || pathSegments.length > 4 || pathSegments[0] !== 'v1')
          throw new Error(
            'Cloudflare AI Gateway API Host is not valid. Please check the API Host field in the Models Setup page.'
          );
    ...
    case 'groq':
      ...
      if (!groqKey)
        throw new Error(
          'Missing Groq API Key. Add it on the UI (Models Setup) or server side (your deployment).'
        );
    ...
    case 'localai':
      ...
      let localAIHost = fixupHost(
        access.oaiHost || env.LOCALAI_API_HOST || DEFAULT_LOCALAI_HOST,
        apiPath
      );
      return {
        headers: {
    ...
    case 'openrouter':
      ...
      if (!orKey || !orHost)
        throw new Error(
          'Missing OpenRouter API Key or Host. Add it on the UI (Models Setup) or server side (your deployment).'
        );
    ...
    case 'perplexity':
      ...
      if (!perplexityKey || !perplexityHost)
        throw new Error(
          'Missing Perplexity API Key or Host. Add it on the UI (Models Setup) or server side (your deployment).'
        );
    ...
    case 'togetherai':
      ...
      if (!togetherKey || !togetherHost)
        throw new Error(
          'Missing TogetherAI API Key or Host. Add it on the UI (Models Setup) or server side (your deployment).'
        );
...
export function openAIChatCompletionPayload(
  ...
  return {
    ...
    ...(functions && {
      functions: functions,
      function_call: forceFunctionName ? { name: forceFunctionName } : 'auto',
    }),
...
async function openaiGETOrThrow<TOut extends object>(
  access: OpenAIAccessSchema,
  apiPath: string /*, signal?: AbortSignal*/
): Promise<TOut> {
  const { headers, url } = openAIAccess(access, null, apiPath);
  return await fetchJsonOrTRPCError<TOut>(
    url,
    'GET',
    headers,
    undefined,
    `OpenAI/${access.dialect}`
  );
}

async function openaiPOSTOrThrow<TOut extends object, TPostBody extends object>(
  access: OpenAIAccessSchema,
  modelRefId: string | null,
  body: TPostBody,
  apiPath: string /*, signal?: AbortSignal*/
  ...
  return await fetchJsonOrTRPCError<TOut, TPostBody>(
    url,
    'POST',
    headers,
    body,
...
function parseChatGenerateFCOutput(
  isFunctionsCall: boolean,
  message: OpenAIWire.ChatCompletion.ResponseFunctionCall
) {
  ...
  let fcArgs: object;
  try {
    fcArgs = JSON.parse(fc.arguments) as object;
  } catch (error: any) {
    throw new TRPCError({
      code: 'INTERNAL_SERVER_ERROR',
...
function parseChatGenerateOutput(
  message: OpenAIWire.ChatCompletion.ResponseMessage,
