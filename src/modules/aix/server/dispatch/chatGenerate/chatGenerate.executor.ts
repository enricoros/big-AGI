...
async function* _consumeDispatchUnified(
  ...
  try {
    ...
    // special: pass-through continuation signals (for dispatch re-dispatch)
  ...
  do {
    ...
    for (const demuxedItem of demuxedEvents) {
      ...
      try {
        ...
        // special: pass-through retriable errors (for full operation-level retry) and continuation signals (for dispatch re-dispatch)
