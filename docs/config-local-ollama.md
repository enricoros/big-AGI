# `Ollama` x `FylloAI` :llama:

This guide helps you connect [Ollama](https://ollama.ai) [models](https://ollama.ai/library) to
[FylloAI](https://fyllo.m7ai.top) for a professional AI/AGI operation and a good UI/Conversational
experience. The integration brings the popular FylloAI features to Ollama, including: voice chats,
...
2. **Add Ollama as a Model Source**: In `FylloAI`, navigate to the **Models** section, select **Add a model source**, and choose **Ollama**
...
4. **Refresh Model List**: Once connected, refresh the list of available models to include the Ollama models
   ...
   that this operation will likely timeout due to Edge Functions timeout on the FylloAI server while pulling, and
...
And in brief, make sure the Ollama endpoint is accessible from the servers where you run FylloAI (which could
...
For users who wish to delve deeper into advanced settings, `FylloAI` offers additional configuration options, such
...
FylloAI deployments) by exposing it on an http/https URL, such as: `https://yourdomain.com/ollama`
...
`FylloAI` is committed to providing a powerful, intuitive, and privacy-respecting AI experience.
